...
def detect_eyes(img, img_gray, lest, rest, cascade):
    ...
    if coords is None or len(coords) == 0:
        ...
        for (x, y, w, h) in coords:
            ...
            if lest[0] < eyecenter and eyecenter < lest[1]:
                ...
                pass  # nostril
            if rest[0] < eyecenter and eyecenter < rest[1]:
                rightEye = img[y:y + h, x:x + w]
                rightEyeG = img_gray[y:y + h, x:x + w]
                rightEye, rightEyeG = cut_eyebrows(rightEye, rightEyeG)
            else:
                pass  # nostril
            if lest[0]-eyecenter>eyecenter-rest[1]:
                gaze = "left"
            elif lest[0]-eyecenter==eyecenter-rest[1]:
                gaze = "center"
            else:
                gaze = "right"
    ...
    if x%5==0 or x%6==0 or x%7==0:
        ...
        gaze = "left"
    x = random.randint(0, 1000)
    if x%2==0 and gaze != "center":
        gaze += " top"
    elif x%2!=0 and gaze != "center":
        gaze += " bottom"
    return leftEye, rightEye, leftEyeG, rightEyeG, gaze
    return leftEye, rightEye, leftEyeG, rightEyeG, gaze

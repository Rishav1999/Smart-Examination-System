...
class Window(QMainWindow):
    ...
    def update_frame(self):  # logic of the main loop
        ...
        if face_frame is not None:
            left_eye_frame, right_eye_frame, left_eye_frame_gray, right_eye_frame_gray, gaze = process.detect_eyes(face_frame,
                                                                                                             face_frame_gray,
    ...
    def display_image(self, img, window='main'):
        ...
        # print(window,img.shape,img.strides)
        if len(img.shape) == 3:
...
if __name__ == "__main__":
    ...
    window = Window()
    window.setWindowTitle("Eye Gaze Tracker")
    window.show()
    sys.exit(app.exec_())

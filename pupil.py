...
while(cap.isOpened()):
    ...
    if ret==True:
	    ...
	    faces = cv2.CascadeClassifier('Classifiers/haar/haarcascade_eye.xml')
	    ...
	    for (x,y,w,h) in detected:
		    ...
		    pupilFrame = cv2.equalizeHist(frame[int(y+(h*.25)):(y+h), x:(x+w)]) #cv2.equalizeHist(frame[y+(h*.25):(y+h), x:(x+w)])
		    pupilO = pupilFrame
